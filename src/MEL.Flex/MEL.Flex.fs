...
module Tuple =
    ...
    let [<Literal>] OriginalFormat = "{OriginalFormat}"
    ...
    type internal TupleLogValuesFormatter(formattable : FormattableString) =
        ...
        let messageFormat = lazy (
            String.Format(formattable.Format, namesTemplate.Value)
        )
        let formattedMessage = lazy (
            String.Format(formattable.Format, args.Value)
        )
        member this.Names
            with get () =
                names.Value
        ...
        interface IReadOnlyList<KeyValuePair<string, obj>> with
            ...
            member this.Count with get() : int = _count
    ...
    type ILogger with
        ...
        member this.LogITrace(ex : Exception, message : FormattableString) =
            this.LogI(LogLevel.Trace, ex, message)
        member this.LogITrace(message : FormattableString) =
            this.LogI(LogLevel.Trace, message)
        ...
        member this.LogIDebug(ex : Exception, message : FormattableString) =
            this.LogI(LogLevel.Debug, ex, message)
        member this.LogIDebug(message : FormattableString) =
            this.LogI(LogLevel.Debug, message)
        ...
        member this.LogIWarning(message : FormattableString) =
            this.LogI(LogLevel.Warning, message)
        ...
        member this.LogIError(ex : Exception, message : FormattableString) =
            this.LogI(LogLevel.Error, ex, message)
        member this.LogIError(message : FormattableString) =
            this.LogI(LogLevel.Error, message)
        ...
        member this.LogICritical(message : FormattableString) =
            this.LogI(LogLevel.Critical, message)

        member this.LogI(logLevel : LogLevel, message : FormattableString) =
            this.LogI(logLevel, 0, null, message)
        ...
        member this.LogI (logLevel : LogLevel, eventId : EventId, ex : Exception, message : FormattableString) =
            this.Log(logLevel, eventId, (TupleLogValuesFormatter message), ex, (fun formatter _ -> formatter.ToString()))
